...
import { Input, Button } from 'aws-amplify-react/lib/AmplifyTheme';
import { unGzipBase64AsJson } from '../helpers/gzip';

const chatBotDataTestAttributes = {
  title: 'chatbot-title',
...
export class Lex2ChatBot extends React.Component<IChatBotProps, IChatBotState> {
  ...
  async lexResponseHandler() {
    ...
    const response = (await Interactions.send(
      ...
      this.props.botName,
      interactionsMessage
    )) as RecognizeUtteranceCommandOutput;
      ...
      if (m.from === 'me') {
        return (
          <div
            key={i}
            style={styles.itemMe}
            data-test={`${chatBotDataTestAttributes.dialog}-${i}`}
          >
  ...
  async sendMessage(text: string) {
    const response: RecognizeTextCommandOutput = (await Interactions.send(
      // @ts-ignore
      this.props.botName,
      text
    ...
    return (
      <FormSection theme={theme}>
        {title && (
          <SectionHeader
            theme={theme}
            data-test={chatBotDataTestAttributes.title}
          >
            <Typography variant={'body1'}>{I18n.get(title)}</Typography>
          </SectionHeader>
        )}
...
function ChatBotTextInput(props) {
  ...
  return (
    <input
      ...
      data-test={chatBotDataTestAttributes.messageInput}
    />
  );
}
...
function ChatBotTextButton(props) {
  ...
  return (
    <button
      ...
      style={styles.button}
      disabled={inputDisabled}
      data-test={chatBotDataTestAttributes.sendMessageButton}
    >
      {I18n.get('Send')}
    </button>
  );
